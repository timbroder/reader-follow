...
function main() {
	...
	Article.prototype = {
		init: function($article) {
			...
			if (GM_getValue("greader_key") === '' || GM_getValue("greader_key") === null || GM_getValue("greader_key") === 'undefined') {
		...
		init_share_button: function() {
			...
			var tempActionbar = this.$action_bar[0];
			var star = tempActionbar.getElementsByClassName('star')[0];
			star.parentNode.insertBefore(this.$share_button, star);
			
			//this.$share_button.insertAfter(this.$action_bar.find(".star"));
			this.$share_button.parentNode.className = this.$share_button.parentNode.className + ' reader-shareable';

			//this.$share_button.onclick = self.share;
	...
	ReaderUI.prototype = {
		get_bar_button: function(text) {
			var outter = document.createElement('span'),
				inner = document.createElement('span');
			
			inner.className = 'link unselectable';
			inner.appendChild(document.createTextNode(text));
			
			outter.className = 'item-link link reader-sharing';
	...
	ReaderSharing.prototype = {
		show_modal: function(force) {
			if (GM_getValue("greader_key") === '' || GM_getValue("greader_key") === null || GM_getValue("greader_key") === 'undefined' || force) {
		...
		update_ui: function () {
			...
			$j('.entry-container:not(.reader-shareable)').each(function () {
				...
				var article = new Article(self.ui, self.loader, $j(this));
